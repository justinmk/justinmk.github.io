...
The solution is to remorselessly truncate the N-dimensional, unitless problem
...
"Three" is adjustable, but the point is there's a fixed number of Most Important
Things that everyone on your team knows each morning are the Most Important
Things.

# Prioritizing a stream

It is less obvious how to apply that method to an ever-growing queue of work.
...
The _crudeness_ of the rubric is critical and intentional: it must be cheap.
Its distraction-cost per item must be near zero. New items flow on-demand
through this triage function.
...
Task management is a subset of project management, a topic beyond the scope of
this essay. There's no simple answer, but open source development suggests that
_passive_ mechanisms are where to look. "Bug bounties" and advocacy are
_active_, and thus don't scale.
